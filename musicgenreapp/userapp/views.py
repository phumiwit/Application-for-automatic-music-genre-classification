...
def usersignup(request):
    ...
    if request.method=="POST":
        ...
        if firstname.isdigit() :
            messages.warning(request, "FirstName ไม่ควรเป็นตัวเลข")
            return redirect("/usersignup")
        if lastname.isdigit():
            messages.warning(request, "LastName ไม่ควรเป็นตัวเลข")
            return redirect("/usersignup")

        if not is_valid:
            messages.warning(request, "email ผิดรูปแบบ")
            return redirect("/usersignup")
        try:
        ...
        if int(age) < 3 or int(age) > 120:
            ...
            return redirect("/usersignup")
        if len(password) < 8:
            messages.warning(request, "password ต้องมีอย่างน้อย 8 ตัว")
            return redirect("/usersignup")

        if not re.search("[0-9]", password):
            messages.warning(request, "password ต้องมีตัวเลขอย่างน้อย 1 ตัว")
            return redirect("/usersignup")
